...
printf '\n' >> $log_location
echo "########### domain_header_report ######################" >> $log_location
echo "find in $log_location/logs/domain_header_report/" >> $log_location
...
mkdir -p ${BASE_FILE_DIR}/logs/domain_header_report/
    ...
    #loop through faers and laers folders
        for laers_faers in faers; do # laers; do
            ...
            for domain in reac; do # drug indi outc reac rpsr ther; do
        ...
        #         aws s3 sync s3://${AWS_S3_BUCKET_NAME}/data/$laers_faers/${domain}/ . --exclude "*" --include "*.txt"
                ...
                for name in ./**/*.txt; do
                    ...
                    if [[ "${#thefilename}" -gt 11 ]]; then
                        ...
                        echo ${name} >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}_after_fixes.csv
                        head -1 --quiet ${name} >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}_after_fixes.csv
                        # echo "name is ${name}";
                        ...
                        if [ $headempty = 0 ]; then
                            ...
                            #create domain log file in ${BASE_FILE_DIR}/logs/domain_header_report/
                                    echo 'this is the header file for this domain '${domain} >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}_after_fixes.csv
                                    echo $thefilename >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}_after_fixes.csv
                                    # printf "\n" >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}_after_fixes.csv
                                    sed -n '2p' < ${name} >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}_after_fixes.csv
                                    printf "\n" >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}_after_fixes.csv
                        else
                            ...
                            # echo ' '
                                    printf "last 2 lines\n" >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}_after_fixes.csv
                                    tail -2 ${name}>> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}_after_fixes.csv
                                    printf "\n\n" >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}_after_fixes.csv
                    ...
                    for name in ./**/*.txt; do
                        ...
                        if [[ "${#thefilename}" -gt 11 ]]; then 
                                ...
                                if [ $headempty = 0 ]; then
                                    ...
                                    #create domain log file in ${BASE_FILE_DIR}/logs/domain_header_report/
                                    echo $thefilename > ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}_new_qtr.txt
                                    # printf "\n" >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}_after_fixes.csv
                                    #add header line to file
                                    echo $header >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}_new_qtr.txt
                                    printf "\n" >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}_new_qtr.txt
                                    #add 2nd line of 1st file that creates
                                    sed -n '2p' < "${name::-4}"_staged_with_lfs_only.txt >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}_new_qtr.txt
                                else
                                    echo 'head is not empty so grabbing from line 2 on'
                                    ...
                                    echo "${name::-4}"_staged_with_lfs_only.txt >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}_new_qtr.txt
                                    echo ' '
                                    printf "\n" >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}_new_qtr.txt
                                    tail -2 "${name::-4}"_staged_with_lfs_only.txt>> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}_new_qtr.txt
                                    printf "\n\n" >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}_new_qtr.txt
                                fi #end if [ $headempty = 0 ]; then
                                    echo ' '
                            else
