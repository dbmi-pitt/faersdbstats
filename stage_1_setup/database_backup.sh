run_date=$(date "+%m%d%Y")
DB_BACKUP_FILENAME="${LOG_FILENAME_ROOT}_stg_1_auto_bk_${run_date}.dump"
...
#PGPASSWORD="${DATABASE_PASSWORD}" pg_dump -Fc -h ${DATABASE_HOST} -p ${DATABASE_PORT} -U ${DATABASE_USERNAME} ${DATABASE_NAME} > ${DB_BACKUP_FILENAME_PATH}
