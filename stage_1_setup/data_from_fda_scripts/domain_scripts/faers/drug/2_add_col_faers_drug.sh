...
cd ${BASE_FILE_DIR}/faersdbstats_data/data_3_sync_columns/laers/drug/
workpwd=${BASE_FILE_DIR}/faersdbstats_data/data_3_sync_columns/laers/drug/
echo pwd now is `pwd`
# exit;
# example workflow:
...
# # ___ 4. run sed '0/$DEMO12Q4.txt$4$12$filename$yr$qtr/$filename$yr$qtr' drug.txt

# aws s3 sync . s3://${AWS_S3_BUCKET_NAME}/data/faers/drug/ --include "*" --exclude "*only.txt"
...
# echo 'set for 05Q2 and 05Q3'
# for name in ./2005/**/*Q2.txt ./2005/**/*Q3.txt; do #trouble area
...
# for name in ./2005/**/*Q3.txt; do
    ...
    if [ "${#name}" -eq 22 ]; then 
        ...
        sed 's/\$/\$\$/5' < $name > ${name::-4}_added_empty_col.txt
        #           \   \      \
...
echo 'this script should have taken *.txt in ./faers/drug/**/ (quarter folders) and added an empty 5th and prod_ai header'
echo ' '
