        ...
        #         aws s3 sync s3://${AWS_S3_BUCKET_NAME}/data/$laers_faers/${domain}/ . --exclude "*" --include "*.txt"
                ...
                for name in ./**/*.txt; do
                    ...
                    if [[ "${#thefilename}" -gt 11 ]]; then
                        ...
                        echo ${name} >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}.txt
                        head -1 --quiet ${name} >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}.txt
                        # echo "name is ${name}";
                        ...
                        if [ $headempty = 0 ]; then
                            ...
                            #create domain log file in ${BASE_FILE_DIR}/logs/domain_header_report/
                                    echo 'this is the header file for this domain '${domain} >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}.txt
                                    echo $thefilename >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}.txt
                                    # printf "\n" >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}.txt
                                    sed -n '2p' < ${name} >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}.txt
                                    printf "\n" >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}.txt
                        else
                            ...
                            # echo ' '
                                    printf "last 2 lines\n" >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}.txt
                                    tail -2 ${name}>> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}.txt
                                    printf "\n\n" >> ${BASE_FILE_DIR}/logs/domain_header_report/${laers_or_faers}_${domain}.txt
