#!/bin/bash
#this was built from the ./s3_data_download.sh so it has lots of things commented out for easier comparison between files
...
printf '\n' >> $log_location
echo "########### domain_line_count_reports ######################" >> $log_location
echo "find in $log_location/logs/domain_line_count_reports/" >> $log_location
...
mkdir -p ${BASE_FILE_DIR}/logs/domain_line_count_reports/
    ...
    #loop through faers and laers folders
        for laers_faers in faers; do # laers; do
        ...
        #         aws s3 sync s3://${AWS_S3_BUCKET_NAME}/data/$laers_faers/${domain}/ . --exclude "*" --include "*.txt"
                ...
                echo $domain ' domains pwd is '`pwd`
                ...
                for name in ./*.txt; do
                    echo 'name is '${name}
                    wordcount=$(wc -l ${name});
                    echo $wordcount >> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}.txt
                    # echo ${name} >> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}.txt                        
                done
                # exit;
                for name in ./**/*.txt; do
                    ...
                    #echo ' '
                    #echo 'thefilename is '$thefilename ' year is '$year 'qtr is '$qtr;

                    #if length of string  thefilename is greater than 11
                    if [[ "${#thefilename}" == 12 ]]; then
                        ...
                        wordcount=$(wc -l ${name});
                        # echo ${name} >> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}.txt
                        # echo $wordcount '$' ${name} >> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}.txt
                        echo $wordcount >> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}.txt
                        # echo ${name} >> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}.txt
                        printf "\n" >> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}.txt
                        # `wc -l ${name} >> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}.txt
                        # echo "name is ${name}";
                        ...
                        if [ $headempty = 0 ]; then
                            ...
                            #create domain log file in ${BASE_FILE_DIR}/logs/domain_line_count_reports/
                                    # echo 'this is the header file for this domain '${domain} >> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}.txt
                                    # echo $thefilename >> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}.txt
                                    # # printf "\n" >> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}.txt
                                    # # sed -n '2p' < ${name} >> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}.txt
                                    # printf "\n" >> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}.txt
                        # else
                            ...
                            # echo ' '
                                    # printf "last 2 lines\n" >> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}.txt
                                    # tail -2 ${name}>> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}.txt
                                    # printf "\n\n" >> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}.txt
...
#                                     #create domain log file in ${BASE_FILE_DIR}/logs/domain_line_count_reports/
#                                     echo $thefilename > ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}_new_qtr.txt
#                                     # printf "\n" >> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}.txt
#                                     #add header line to file
#                                     echo $header >> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}_new_qtr.txt
#                                     printf "\n" >> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}_new_qtr.txt
#                                     #add 2nd line of 1st file that creates
#                                     sed -n '2p' < "${name::-4}"_staged_with_lfs_only.txt >> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}_new_qtr.txt
#                                 else
#                                     echo 'head is not empty so grabbing from line 2 on'
...
#                                     echo "${name::-4}"_staged_with_lfs_only.txt >> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}_new_qtr.txt
#                                     echo ' '
#                                     printf "\n" >> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}_new_qtr.txt
#                                     tail -2 "${name::-4}"_staged_with_lfs_only.txt>> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}_new_qtr.txt
#                                     printf "\n\n" >> ${BASE_FILE_DIR}/logs/domain_line_count_reports/${laers_or_faers}_${domain}_new_qtr.txt
#                                 fi #end if [ $headempty = 0 ]; then
#                                     echo ' '
#                             else
...
echo "find in $log_location/logs/domain_line_count_reports/"