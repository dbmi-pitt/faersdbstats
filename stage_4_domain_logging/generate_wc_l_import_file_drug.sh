#!/bin/bash
# /home/pentaho-secondary/projects-brb265-2024/faersdbstats/faersdbstats/stage_4_domain_logging/generate_wc_l_import_file_drug.sh
...
BASE_FILE_DIR="$BASE_FILE_DIR/data_from_s3/faers/drug"

# Output file for all years
OUTPUT_DIR="$BASE_FILE_DIR/qa"
OUTPUT_FILE="$OUTPUT_DIR/drug_wc_import.psv"
...
if [[ "$filename" =~ _staged_with_lfs_only\.txt$ || "$filename" =~ ^old\.txt$ || "$filename" =~ ^ol\.txt$ || "$filename" =~ bak ]]; then
    ...
    # Note: yr is two-digit; convert to 4-digit for comparison
    y4=$((2000 + yr))
