...
if [ "${LOAD_ALL_TIME}" = 1 ]; then
    ...
    for laers_faers in faers; do # laers; do
        ...
        # echo outer loop pwd is `pwd`
            ...
            for domain in reac; do #demo drug indi outc reac rpsr ther; do
                    for file in ./$faers_laers/$domain/**/*.txt; do
                ...
                for name in ./**/*.txt; do
                    ...
                    # thefilenamedata='';
                    #chop off first 10 of name #DEMO12Q4.txt
                    thefilename=${name: 10}
                    #grab 8 chars past 10th #DEMO12Q4
                    ...
                    # DRUG12Q4_staged_with_lfs_only_new_header.txt <44 chars
                    # DRUG12Q4_staged_with_lfs_only.txt <33 chars
                    if [[ "${#thefilename}" -gt 11 ]]; then 
                        ...
                        if [ $headempty = 0 ]; then
                            ...
                            #this is a guess to fix the header of *_staged...
                            sed -i "0,/$thefilenamedata/{s/$thefilenamedata//}" "${name::-4}"_staged_with_lfs_only.txt
                            
                            #next guess to fix the headers of *_staged...
                            #sed -i "0,/$thefilenamedata/\$thefilenamedata\$yr\$qtr//}" "${name::-4}"_staged_with_lfs_only.txt
