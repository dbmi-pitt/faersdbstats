...
from casedemo_legacy a
  left outer join country_code e on upper(a.reporter_country) = upper(e.country_name);
------------------------------
...
where caseid is not null
  ...
  and age is not null
  and sex is not null
  and reporter_country is not null
group by caseid,
  age,
  sex,
  reporter_country;
...
where a.caseid = d.caseid
  and a.age = d.age
  and a.sex = d.sex
  and a.reporter_country = d.reporter_country
  and a.caseid is not null
  and a.event_dt is null
...
where a.caseid = d.caseid
  ...
  and a.age is not null
  and a.sex is not null
  and a.reporter_country is null;
------------------------------
...
select caseid,
  case
    when isr is not null then null
    else primaryid
  end as primaryid,
  isr
from (
    ...
    where caseid is not null
      ...
      and sex is not null
      and reporter_country is not null
      and drugname_list is not null
      and reac_pt_list is not null
  ) a
where a.row_num = 1
union
select caseid,
...
where caseid is null
  ...
  or age is null
  or sex is null
  or reporter_country is null
  or drugname_list is null
  or reac_pt_list is null;