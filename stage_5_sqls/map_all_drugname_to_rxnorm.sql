...
set
	search_path = staging_vocabulary;

drop index if exists vocab_concept_name_ix;

create index vocab_concept_name_ix on staging_vocabulary.concept(
	vocabulary_id,
	standard_concept,
	upper(concept_name),
	concept_id
...
from
	(
		select
			distinct drugname as drug_name_original,
...
update
	drug_regex_mapping
set
	drug_name_clean = regexp_replace(
		drug_name_clean,
		'(.*)(\W|^)\(TABLETS?\)|TABLETS?(\W|$)',
		'\1\2',
		'gi'
...
UPDATE
	drug_regex_mapping a
SET
	update_method = 'regex remove keywords',
	concept_id = b.concept_id
FROM
	staging_vocabulary.concept b
WHERE
...
UPDATE
	drug_regex_mapping a
SET
	update_method = 'regex EU drug name to active ingredient',
	drug_name_clean = upper(b.active_substance)
FROM
	eu_drug_name_active_ingredient_mapping b
WHERE
	upper(a.drug_name_clean) = upper(b.brand_name)
	AND a.concept_id is null;
...
update
	drug_regex_mapping a
set
	update_method = 'regex EU drug name in parentheses to active ingredient',
	drug_name_clean = upper(b.active_substance)
from
	eu_drug_name_active_ingredient_mapping b
where
	upper(
		regexp_replace(a.drug_name_clean, '.* \((.*)\)', '\1', 'gi')
	) = upper(b.brand_name)
	and a.concept_id is null
...
UPDATE
	drug_regex_mapping a
SET
	update_method = 'regex ingredient name in parentheses',
	concept_id = b.concept_id,
	drug_name_clean = upper(b.concept_name)
FROM
	staging_vocabulary.concept b
WHERE
	b.vocabulary_id = 'RxNorm'
...
update
	drug_regex_mapping
set
	drug_name_clean = regexp_replace(drug_name_clean, '[ \.\,]$', '', 'gi')
where
	concept_id is null;
...
UPDATE
	drug_regex_mapping a
SET
	update_method = 'regex trailing space or period chars',
	concept_id = b.concept_id
FROM
	staging_vocabulary.concept b
WHERE
...
update
	drug_regex_mapping
set
	drug_name_clean = regexp_replace(drug_name_clean, '(\S) +', '\1 ', 'gi')
where
	concept_id is null;
...
UPDATE
	drug_regex_mapping a
SET
	update_method = 'regex remove multiple white space',
	concept_id = b.concept_id
FROM
	staging_vocabulary.concept b
WHERE
...
UPDATE
	drug_regex_mapping a
SET
	update_method = 'regex remove trailing spaces',
	concept_id = b.concept_id
FROM
	staging_vocabulary.concept b
WHERE
...
UPDATE
	drug_regex_mapping a
SET
	update_method = 'regex remove leading spaces',
	concept_id = b.concept_id
FROM
	staging_vocabulary.concept b
WHERE
...
UPDATE
	drug_regex_mapping a
SET
	update_method = 'regex remove single quotes',
	concept_id = b.concept_id
FROM
	staging_vocabulary.concept b
WHERE
...
UPDATE
	drug_regex_mapping a
SET
	update_method = 'regex remove ^*$? punctuation chars',
	concept_id = b.concept_id
FROM
	staging_vocabulary.concept b
WHERE
...
UPDATE
	drug_regex_mapping a
SET
	update_method = 'regex change forward slash to back slash',
	concept_id = b.concept_id
FROM
	staging_vocabulary.concept b
WHERE
...
UPDATE
	drug_regex_mapping a
SET
	update_method = 'regex remove spaces before closing parenthesis',
	concept_id = b.concept_id
FROM
	staging_vocabulary.concept b
WHERE
...
UPDATE
	drug_regex_mapping a
SET
	update_method = 'regex remove (unknown)',
	concept_id = b.concept_id
FROM
	staging_vocabulary.concept b
WHERE
...
update
	drug_regex_mapping
set
	drug_name_clean = regexp_replace(drug_name_clean, ' *blinded *', '', 'gi')
where
	concept_id is null;
...
UPDATE
	drug_regex_mapping a
SET
	update_method = 'regex remove blinded',
	concept_id = b.concept_id
FROM
	staging_vocabulary.concept b
WHERE
...
update
	drug_regex_mapping a
SET
	drug_name_clean = regexp_replace(drug_name_clean, '\/\d+\/\ *', '', 'gi')
where
	concept_id is null
...
UPDATE
	drug_regex_mapping a
SET
	update_method = 'regex remove /nnnnn/',
	concept_id = b.concept_id
FROM
	staging_vocabulary.concept b
WHERE
...
update
	drug_regex_mapping a
set
	drug_name_clean = 'MULTIVITAMIN PREPARATION'
where
	drug_name_clean like '%VITAMIN%'
				...
				from
					(
						...
						from
							(
								select
									upper(concept_name) as concept_name,
									concept_id
								from
									staging_vocabulary.concept b
								where
...
update
	drug_regex_mapping_words c
SET
	update_method = 'multiple ingredient match',
	concept_name = b.concept_name,
	concept_id = b.concept_id
from
	(
		select
			distinct a.drug_name_original,
			max(upper(b1.concept_name)) as concept_name,
			max(b1.concept_id) as concept_id
		from
			drug_mapping_multi_ingredient_list a
			inner join rxnorm_mapping_multi_ingredient_list b1 on a.ingredient_list = b1.ingredient_list
		group by
			a.drug_name_original
	) b
where
	c.drug_name_original = b.drug_name_original
...
update
	drug_regex_mapping_words c
SET
	update_method = 'single ingredient match',
	concept_name = b.concept_name,
	concept_id = b.concept_id
from
	(
		select
			...
			max(b1.concept_id) as concept_id
		from
			drug_mapping_single_ingredient_list a
...
update
	drug_regex_mapping_words c
SET
	update_method = 'brand name match',
	concept_name = b.concept_name,
	concept_id = b.concept_id
from
	(
		select
			...
			max(b1.concept_id) as concept_id
		from
			drug_mapping_brand_name_list a
...
update
	drug_regex_mapping c
SET
	update_method = b.update_method,
...
UPDATE
	drug_ai_mapping a
SET
	update_method = 'drug active ingredients',
	concept_id = b.concept_id
FROM
	staging_vocabulary.concept b
WHERE
...
SET
	...
	nda_ingredient = nda_ingredient.ingredient,
	concept_id = b.concept_id
FROM
	staging_vocabulary.concept b
	inner join nda_ingredient on upper(b.concept_name) = nda_ingredient.ingredient
WHERE
	b.vocabulary_id = 'RxNorm'
...
from
	(
		select
			distinct b.primaryid,
			b.isr,
			drug_seq,
			role_cod,
			drugname as drug_name_original,
			cast(null as varchar) as lookup_value,
			cast(null as integer) as concept_id,
			cast(null as varchar) as update_method
		from
			drug a
			inner join unique_all_case b on a.primaryid = b.primaryid
		where
...
UPDATE
	combined_drug_mapping a
SET
	update_method = b.update_method,
	lookup_value = drug_name_clean,
	concept_id = b.concept_id
FROM
	drug_regex_mapping b
WHERE
	upper(a.drug_name_original) = upper(b.drug_name_original)
	and a.concept_id is null
...
SET
	update_method = b.update_method,
	lookup_value = b.concept_name,
	concept_id = b.concept_id
FROM
	drug_usagi_mapping b
WHERE
	upper(a.drug_name_original) = upper(b.drug_name_original)
	and a.concept_id is null
	and b.concept_id is not null;
...
update
	combined_drug_mapping
set
	update_method = 'unknown drug'
where
	upper(drug_name_original) ~* '^UNKNOWN.*'
...
set
	update_method = 'unknown drug'
where
	upper(drug_name_original) ~* '^OTHER.*'
	and update_method is null;
...
set
	update_method = 'unknown drug'
where
	upper(drug_name_original) ~* '^UNSPECIFIED.*'
	and update_method is null;